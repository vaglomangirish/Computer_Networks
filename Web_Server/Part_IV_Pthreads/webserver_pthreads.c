// TCP Multithreaded Server handling non-persistent connections.
...
#include <pthread.h>    //Requires pthread linking
...
/*
 * This will handle separate connections from each client.
 * A separate TCP connection in separate thread.
 */
void *thread_handler(void *client_sfd)
{
  ...
  int x, len;
  int csfd = *(int *)client_sfd;

  bzero(buffer, BUFFERSIZE);
  len = read(csfd ,buffer, BUFFERSIZE-1);
  ...
  //printf("\n4");
  len = write(csfd, response, strlen(response));
  //printf("\n5");
 ...
 * specified port, and creates threads for every request from client.
 */
void server_run(char *portno)
{
  ...
  struct sockaddr_in serv_address, cli_address;
    ...
    int server_sfd, client_sfd, port, *new_sock;
    ...
    printf("Server started, listening on port %s", portno);
  ...
  while(1)
  {
    ...
    client_sfd = accept(server_sfd, (struct sockaddr *) &cli_address, &clen);
    ...
    printf("\nClient connection received at: %ld seconds and %ld micro-seconds\n", req_recv.tv_sec, req_recv.tv_usec);

    pthread_t req_thread;
    new_sock = malloc(1);
    *new_sock = client_sfd;

    if( pthread_create( &req_thread , NULL ,  thread_handler , (void*) new_sock) < 0)
    {
        report_error("\nERR: Could not create thread.", &server_sfd, &client_sfd);
    }
